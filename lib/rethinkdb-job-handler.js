...
const RethinkDBConfig = require('./config/rethinkdb_defaults')

class RethinkDBJobQueue {

  constructor (connection, server) {
    this.connection = Object.assign(RethinkDBConfig, connection)
    this.Queues = {}
    this.server = server
  ...
  importJobs (path) {
    ...
    _.forEach(files, (file) => {
      ...
      if (file_path && Fs.lstatSync(file_path).isFile()) {
        ...
        if (Job) {
          Job = new Job()
          const Queue = new JobQueue(this.connection, Job.queueOptions())
