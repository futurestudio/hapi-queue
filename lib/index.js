 ...
 */
const initializeJobs = function initializeJobs (jobsPath) {
    ...
    _.keys(jobs).forEach(function (job) {
        ...
        Queue.process(jobName, jobConcurrency, jobConfig.process)
    })
}
...
const createJob = function createJob (job) {
    return Queue.create(job.name, job.data)
}
